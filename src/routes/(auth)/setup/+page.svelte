	...
	async function handleSetup() {
		...
		try {
			const res = await fetch('/api/auth/setup', {
				...
				body: JSON.stringify({ flatNumber, displayName, pin, token: data.token })
			});
...
{#if data.locked}
	<Card.Root class="shadow-sm">
		<Card.Header class="pb-2 text-center">
			<div class="bg-muted mx-auto mb-2 flex h-12 w-12 items-center justify-center rounded-xl">
				<LockKeyhole class="text-muted-foreground h-6 w-6" />
			</div>
			<Card.Title class="text-2xl font-bold tracking-tight">Configuration requise</Card.Title>
			<Card.Description>
				L'application n'est pas encore configurée. Contactez l'administrateur du serveur pour obtenir un lien
				d'installation.
			</Card.Description>
		</Card.Header>
	</Card.Root>
		...
		<Card.Content>
			<form onsubmit={handleSetup} class="space-y-4">
				<div class="space-y-2">
					...
					<Input id="flat" type="text" placeholder="ex. B23" bind:value={flatNumber} required />
				</div>
				<div class="space-y-2">
